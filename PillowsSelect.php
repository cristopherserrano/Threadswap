...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <title> ThreadSwap Pillows </title>
        <style>
        body {
          margin:auto;
        }
...
<nav class="w3-sidebar w3-bar-block w3-white w3-collapse w3-top" style="z-index:3;width:250px" id="mySidebar">
  ...
  <div class="w3-padding-64 w3-large w3-text-grey" style="font-weight:bold">
    ...
    <a href="BlanketssSelect.php" class="w3-bar-item w3-button">Blankets</a>
    <a href="PillowsSelect.php" class="w3-bar-item w3-button">Pillows</a>
</nav>
...
$sql="SELECT * FROM Pillows ORDER BY item_id";
$result = mysqli_query($con,$sql);
// Print the data from the table row by row
$col = 0;
while($row = mysqli_fetch_array($result)) {
  $Pillows .= '<li>';
  $Pillows .= '<img src="'. $row['imageLink'] .'" style="width:230px;height:230px;"/>';
  $Pillows .= '<h4>' . $row['name'] . '</h4>';
  $Pillows .= '<p>' . $row['size'];
  $Pillows .= " " . $row['description'];
  $Pillows .= '<p>' . $row['brand'];
  $Pillows .= '<p>' . $row['color'] . '</p>';

  $Pillows .= '<div class=details>';
  $Pillows .= '<p>' . "$" . $row['price'];
