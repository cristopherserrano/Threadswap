...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
        <title> ThreadSwap Blankets </title>
        <style>
        body {
          margin:auto;
        }
...
$sql="SELECT * FROM Blankets INNER JOIN clothing ON Blankets.item_id = clothing.clothingID ORDER BY Blankets.item_id";
$result = mysqli_query($con,$sql);
// Print the data from the table row by row
$col = 0;
while($row = mysqli_fetch_array($result)) {
  $Blankets .= '<li>';
  $Blankets .= '<img src="'.$row['imageLink'].'" style="width:230px;height:230px;"/>';
  $Blankets .= '<h4>'.$row['name'].'</h4>';
  $Blankets .= '<p>' . $row['size'];
  $Blankets .= " " . $row['description'];
  $Blankets .= '<p>' . $row['brand'];
  $Blankets .= '<p>' . $row['color'] . '</p>';
  $Blankets .= '<p>' . "$" . $row['price']. '</p>';
  $Blankets .= '<p>' ."stock:".$row['Stock']. '</p>';
  $Blankets .= '<a href="buyNow.php?item_id='.$row['item_id'].'" style="color:blue">BUY</a>';
  $Blankets .= '</li>';
}
mysqli_close($con);
$Blankets .= '</ul>';
